					...
					>>>>>>>>>>>>[ если в $12 не ноль; то мы не на последней строчке
						...
						>>>>>>>>>>>>[ если в $12 не ноль; то мы не на левой крайней ячейке
							...
							>>>>>>>>>>>>-[ если в $12 было больше единицы; то мы не в правой крайней ячейке
								...
								>>>>>>>>>>>>[-]>[-]<<<<<<<<<<<<< <[> >>>>>>>>>>>>>+<<<<<<<<<<<<< <-]> >>>>>>>>>>>>>[<+<<<<<<<<<<<<<+>>>>>>>>>>>>>>-]<<<<<<<<<<<<< < > скопировали в $12 значение левого соседа; текущая ячейка $0
								>>>>>>>[-]+>>>>>
								-[ if; в $12 было больше единицы
									-[ if; в $12 было больше двух
										<<<<<[-]>>>>>[-] выходим из if и else; не увеличиваем счетчик
									]
									<<<<<[ else; в $12 была двойка; увеличим счетчик живых соседей
										>+<
										[-]
									]>>>>>
									<<<<<[-]>>>>>[-] выход из if и else
								]
								<<<<<[ else; в $12 была единица; увеличим счетчик живых соседей
									>+< увеличиваем счетчик живых соседей
									[-]
								]>>>>>
								<<<<<<<<<<<< вернулись в $0
								!просмотр правого соседа!
								>>>>>>>>>>>>[-]>[-]>>> перешли к правому соседу
								[<<<+<+>>>>-]<<<[>>>+<<<-]>>> скопировали его в $12
								<<<<<<<<<<<<<<<< вернулись в $0
								>>>>>>>[-]+>>>>> перешли в $12; записали флаг else
								-[ if; в $12 было больше единицы
									-[ if; в $12 было больше двух
										<<<<<[-]>>>>>[-] выходим из if и else; не увеличиваем счетчик
									]
								...
								<<<<<<<<<<<< вернулись в $0
								!просмотр нижних соседей!
								>>>> >>>>>>>>[-]<<<<<<<< [>>>>>>>>+<<<<<<<<-]>>>>>>[-]>[-]>[<+<+<<<<<<+>>>>>>>>-]<-<-<<<<<<<<<< скопировали в $10 и $11 ширину строки минус 1
								>>>>>>>>>>>[ пройдем вперед на ширина минус один ячеек; после чего посмотрим текущую и две следующих после нее ячейки
									<<<<<<<<<<< текущая ячейка $0
									!сдвиг вправо!
									>>>>>>>>>>>>>>[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<[>+<-]<
									>>>>>>>>>>>>>>>[<<<<<<<<<<<<<<<+>>>>>>>>>>>>>>>-]<<<<<<<<<<<<<<< сдвиг вправо сделан
									> вправо; потому что сдвигались
									>>>>>>>>>>>- уменьшаем счетчик
								]<<<<<<<<<<<
								!просмотр текущей и двух следующих ячеек!
								это точно ячейка $0; в $0 значение левого нижнего соседа проверяемой клетки
								>>>>>>>>>>>>[-]>[-]<<<<<<<<<<<<< >>>>>>>>>>>>>>>[<<+<+>>>-]<<[>>+<<-]>><<<<<<<<<<<<<<< в $12 копия значения ячейки; вернулись в $0
								>>>>>>>[-]+>>>>> перешли в $12; записали флаг else
								-[ if; в $12 было больше единицы
									-[ if; в $12 было больше двух
										<<<<<[-]>>>>>[-] выходим из if и else; не увеличиваем счетчик
								...
								<<<<<<<<<<<< вернулись в $0
								!проверим нижнюю среднуюю ячейку!
								>>>>>>>>>>>>[-]>[-]<<<<<<<<<<<<< >>>>>>>>>>>>>>>>[<<<+<+>>>>-]<<<[>>>+<<<-]>>><<<<<<<<<<<<<<<< в $12 копия значения ячейки; вернулись в $0
								>>>>>>>[-]+>>>>> перешли в $12; записали флаг else
								-[ if; в $12 было больше единицы
									-[ if; в $12 было больше двух
								...
								<<<<<<<<<<<< вернулись в $0
								!нижние соседи проверены; возвращаемся назад!
								>>>>>>>>>>[
									<<<<<<<<<< текущая ячейка $0
									!сдвиг влево!
									[>>>>>>>>>>>>>>>+<<<<<<<<<<<<<<<-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]>[<+>-]<<<<<<<<<<<<<< сдвиг влево сделан
									< влево; потому что сдвигались
									>>>>>>>>>>- уменьшить счетчик
								]<<<<<<<<<<
								>>>>>>>[-]<<<<<<< обнуление флага else
								!просмотр верхних соседей!
								!изменить значение ячейки в зависимости от количества живых соседей!
								текущая ячейка $0
								>>>>>>>[-]+>>>>>[-]>[-]<<<<<<<<<<<<<[>>>>>>>>>>>>>+<<<<<<<<<<<<<-]>>>>>>>>>>>>>[<+<<<<<<<<<<<<+>>>>>>>>>>>>>-]<<<<<<<<<<<<< копируем в $12 значение текущей ячейки; флаг else
								>>>>>>>>>>>> перешли в $12
								>[-]+< ?
								<<<<<<<<<<<<[-]>>>>>>>>>>>> >[<<<<<<<<<<<<<+>>>>>>>>>>>>>-]< копируем новое значение ячейки; работает
								<<<<<<<<<<<< вернулись в $0
